...
const InterviewPrep = () => {
  ...
  const generateConceptExplanation = async (question) => {
    try {
      ...
      if (error.response?.status === 503) {
        setErrorMsg("AI is rate-limited. Please wait a moment and try again.");
      } else {
  ...
  const uploadMoreQuestions = async () => {
    try {
      ...
      if (error.response?.status === 503) {
        setErrorMsg("AI is rate-limited. Please wait a few seconds and try again.");
  ...
  return (
    <DashboardLayout>
      ...
      <div className="container mx-auto pt-4 pb-4 md:px-0">
        ...
        {errorMsg && (
          <p className="flex gap-2 text-sm text-amber-600 font-medium mt-3">
              ...
              {sessionData?.questions?.map((data, index) => (
                ...
                >
                  ...
                  {sessionData?.questions?.length === index + 1 && (
                    <div className="flex items-center justify-center mt-5">
                      <button
                        className="flex items-center gap-3 text-sm text-white font-medium bg-black px-5 py-2 mr-2 rounded text-nowrap cursor-pointer disabled:opacity-60"
                        disabled={isLoading || isUpdateLoader}
                        onClick={uploadMoreQuestions}
                      >
                        {isUpdateLoader ? (
